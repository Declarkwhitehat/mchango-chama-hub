...
const Profile = () => {
  ...
  const handleEnableNativeBiometric = async () => {
    ...
    try {
      ...
      if (result.success) {
        // Use helper to get a fresh session with refresh token
  ...
  return (
    <Layout>
      <div className="container px-3 sm:px-4 py-4 sm:py-6 pb-20 sm:pb-24 max-w-2xl mx-auto space-y-4 sm:space-y-6">
                ...
                <div className="space-y-3 sm:space-y-4 py-4">
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      ...
                      >
                        {showNewPassword ? <EyeOff className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-muted-foreground" /> : <Eye className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-muted-foreground" />}
                      </Button>
                    </div>
                  </div>
                  <div className="space-y-2">
                  ...
                  <Button onClick={handlePasswordUpdate} className="w-full text-sm sm:text-base" disabled={isUpdatingPassword}>
              ...
              <div className="space-y-4 pt-4 border-t border-border">
                ...
                {isLoadingCredentials ? (
                  ...
                  <div className="space-y-4">
                    ...
                    <Button onClick={handleAddBiometric} disabled={isAddingBiometric} className="w-full text-sm sm:text-base">
                  ...
                  <div className="space-y-3">
                    {credentials.map((credential) => (
                      <div key={credential.id} className="flex items-center justify-between p-3 sm:p-4 bg-muted/50 rounded-lg border border-border">
                        <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
                          <Fingerprint className="h-4 w-4 sm:h-5 sm:w-5 text-primary flex-shrink-0" />
                          <div className="min-w-0 flex-1">
                            <p className="font-medium text-sm sm:text-base truncate">{credential.device_name || 'Biometric Device'}</p>
                            <p className="text-xs text-muted-foreground">Registered {format(new Date(credential.created_at), 'MMM d, yyyy')}</p>
                            {credential.last_used_at && (
                              <p className="text-xs text-muted-foreground">Last used {format(new Date(credential.last_used_at), 'MMM d, yyyy')}</p>
                            )}
                          </div>
                        </div>
                        <Button variant="ghost" size="sm" onClick={() => setCredentialToDelete(credential.credential_id)} className="flex-shrink-0">
                          <Trash2 className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-destructive" />
                        </Button>
                      </div>
                    ))}
                    <Button onClick={handleAddBiometric} disabled={isAddingBiometric} variant="outline" className="w-full text-sm sm:text-base">
                      <Plus className="mr-2 h-3.5 w-3.5 sm:h-4 sm:w-4" />
        ...
        <Card>
          <CardContent className="pt-4 sm:pt-6 px-4 sm:px-6 pb-4 sm:pb-6">
            <Button variant="destructive" className="w-full text-sm sm:text-base" onClick={handleLogout}>
      ...
      <AlertDialog open={!!credentialToDelete} onOpenChange={() => setCredentialToDelete(null)}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={handleDeleteBiometric} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">
              Remove Device
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      ...
      <AlertDialog open={showDisableBiometricDialog} onOpenChange={setShowDisableBiometricDialog}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={handleDisableNativeBiometric} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">
              Disable Fingerprint
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
