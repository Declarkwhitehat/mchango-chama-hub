...
Deno.test("daily cycle advances to the next Kenya day at 00:01–21:00 EAT", () => {
  ...
  assertEquals(result.endDate.toISOString(), "2026-08-15T18:00:00.000Z");
});
...
Deno.test("weekly cycle covers seven Kenya calendar days", () => {
  ...
  assertEquals(result.startDate.toISOString(), "2026-08-14T21:01:00.000Z");
  assertEquals(result.endDate.toISOString(), "2026-08-21T18:00:00.000Z");
});
...
Deno.test("monthly chosen day crosses month end without fixed 30-day drift", () => {
  ...
  assertEquals(result.endDate.toISOString(), "2026-02-28T18:00:00.000Z");
});
...
Deno.test("twice-monthly alternates configured calendar days", () => {
  ...
  assertEquals(result.endDate.toISOString(), "2026-08-25T18:00:00.000Z");
});
...
Deno.test("every-N-days uses inclusive cycle windows", () => {
  ...
  assertEquals(result.endDate.toISOString(), "2026-08-17T18:00:00.000Z");
});
Deno.test("twice-weekly alternates the two chosen weekdays (Mon & Thu)", () => {
  ...
  assertEquals(first.endDate.toISOString(), "2026-08-17T18:00:00.000Z");
  ...
  assertEquals(second.endDate.toISOString(), "2026-08-20T18:00:00.000Z");
  ...
  assertEquals(third.endDate.toISOString(), "2026-08-24T18:00:00.000Z");
});
...
Deno.test("twice-weekly handles Sunday/Monday pair across year boundary", () => {
  ...
  assertEquals(first.endDate.toISOString(), "2027-01-03T18:00:00.000Z");
  ...
  assertEquals(second.endDate.toISOString(), "2027-01-04T18:00:00.000Z");
});
...
Deno.test("twice-weekly falls back safely when days are missing or identical", () => {
  ...
  assertEquals(identical.endDate.toISOString(), "2026-08-19T18:00:00.000Z");
});
