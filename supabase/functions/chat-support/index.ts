...
- **Payout confirmation SMS**: The recipient of a chama/welfare/mchango/organization payout receives an SMS the moment Safaricom confirms the transfer. Format: "Confirmed you have received KES <amount> from <group name> <type>. Mpesa Ref: <code>. Sisi tuko pamoja, je wewe?" — so they always know which group sent the money.
- **Chama auto-continue (NEW)**: When a chama cycle completes, members no longer need to submit rejoin requests. 24 hours after `cycle_complete`, the chama AUTOMATICALLY starts a new cycle with all members who cleared their debts. Members with outstanding debts are auto-removed (and SMS'd). Debtors get a 24h warning to clear or be removed. If fewer than `min_members` are debt-free, the chama waits in `cycle_complete` until enough have cleared.
- **Mid-stream chama joins (NEW)**: New members can now request to join an `active` chama (not only `pending` or `cycle_complete`). After manager approval, they join with `status=inactive` until their first contribution lands; if they miss the first payment they are auto-removed.
- **Manager advisory on creation**: The Create Chama page now displays an amber banner reminding the manager to invite only people they personally know — pools real money and reputation is on the line.
