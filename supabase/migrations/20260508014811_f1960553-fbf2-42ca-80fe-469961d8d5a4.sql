...
BEGIN
  ...
  BEGIN
    ...
    IF FOUND THEN v_has_payout := true; END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'data', jsonb_build_object(
      ...
      'current_cycle', CASE WHEN v_has_payment THEN
        jsonb_build_object(
      ...
      'payout_schedule', CASE WHEN v_has_payout AND v_payout.position_in_queue IS NOT NULL THEN
        jsonb_build_object(
