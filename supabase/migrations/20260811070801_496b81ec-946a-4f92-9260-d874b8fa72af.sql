...
BEGIN
  ...
  WITH unified AS (
    SELECT d.id, 'Organization Donation'::text AS type, COALESCE(o.name,'Unknown')::text AS source_name,
           COALESCE(d.gross_amount, d.amount)::numeric AS amount, d.net_amount::numeric AS net_amount,
           d.commission_amount::numeric AS commission,
           d.payment_status::text AS status, d.mpesa_receipt_number::text AS mpesa_receipt,
           d.payment_reference::text AS reference, d.created_at, d.completed_at
      FROM organization_donations d LEFT JOIN organizations o ON o.id = d.organization_id
     WHERE d.user_id = p_user_id
    UNION ALL
    SELECT d.id, 'Mchango Donation'::text, COALESCE(m.title,'Unknown')::text,
           COALESCE(d.gross_amount, d.amount)::numeric, d.net_amount::numeric, d.commission_amount::numeric,
           d.payment_status::text, d.mpesa_receipt_number::text, d.payment_reference::text, d.created_at, d.completed_at
      FROM mchango_donations d LEFT JOIN mchango m ON m.id = d.mchango_id
     WHERE d.user_id = p_user_id
    UNION ALL
    SELECT d.id, 'Welfare Contribution'::text, COALESCE(w.name,'Unknown')::text,
           d.gross_amount::numeric, d.net_amount::numeric, d.commission_amount::numeric,
           d.payment_status::text, d.mpesa_receipt_number::text, d.payment_reference::text, d.created_at, d.completed_at
      FROM welfare_contributions d LEFT JOIN welfares w ON w.id = d.welfare_id
     WHERE d.user_id = p_user_id
    UNION ALL
    SELECT c.id, 'Chama Contribution'::text, COALESCE(ch.name,'Unknown')::text,
           c.amount::numeric, NULL::numeric, NULL::numeric,
           c.status::text, c.mpesa_receipt_number::text, c.payment_reference::text, c.created_at, NULL::timestamptz
