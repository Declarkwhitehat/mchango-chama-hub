...
-- 3. Update saving_group_deposits table (this is the "savings" table)
...
-- Add index for saved_for relationships
...
CREATE TABLE IF NOT EXISTS saving_group_transactions (
  ...
  reference_id UUID, -- links to specific savings/loan/repayment record
  notes TEXT,
...
CREATE TABLE IF NOT EXISTS saving_group_profits (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  group_id UUID NOT NULL REFERENCES saving_groups(id) ON DELETE CASCADE,
  cycle_period TEXT NOT NULL, -- e.g., "2025-01" for January 2025
...
CREATE TABLE IF NOT EXISTS saving_group_profit_shares (
  ...
  savings_ratio NUMERIC NOT NULL DEFAULT 0, -- percentage of total savings
...
-- Create indexes for profit shares
...
CREATE TABLE IF NOT EXISTS company_earnings (
  ...
  reference_id UUID, -- links to specific transaction
  description TEXT,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT TIMEZONE('utc', NOW()) NOT NULL
);

-- Create indexes for company earnings
...
-- Policies for saving_group_transactions
...
-- Policies for saving_group_profits
...
-- Policies for saving_group_profit_shares
...
-- Function to calculate available loan pool (30% of total savings minus active loans)
...
-- Function to record company earnings
