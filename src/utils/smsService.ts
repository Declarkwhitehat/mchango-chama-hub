...
export const SMS_TEMPLATES = {
  accountCreated: (name: string) =>
    `Welcome, ${name}! Your account has been created successfully. Log in to explore Chamas, Mchangos, and more.${STOP_LINE}`,

  chamaCreated: (chamaName: string) =>
    `Your Chama "${chamaName}" has been created. Share the invite link with members to get started.${STOP_LINE}`,
  ...
  fullPayoutNotice: (amount: number, chamaName: string, requiresVerification: boolean) =>
    `Payout of KES ${amount.toFixed(2)} from "${chamaName}" has been processed. All members contributed. ${requiresVerification ? 'Pending admin approval.' : 'Funds will arrive shortly.'}${STOP_LINE}`,

  partialPayoutNotice: (amount: number, chamaName: string, paidCount: number, totalCount: number, requiresVerification: boolean) =>
    `Payout of KES ${amount.toFixed(2)} from "${chamaName}" processed (${paidCount}/${totalCount} members paid). ${requiresVerification ? 'Pending admin approval.' : 'Funds will arrive shortly.'}${STOP_LINE}`,

  latePaymentCredit: (amount: number, nextCycleDate: string) =>
    `Your payment of KES ${amount.toFixed(2)} was received after the daily cut-off. It has been credited to your next cycle on ${nextCycleDate}.${STOP_LINE}`,

  managerMissedPaymentAlert: (memberName: string, memberCode: string, chamaName: string, missedCount: number) =>
    `Alert — ${memberName} (${memberCode}) has missed ${missedCount} contribution(s) in "${chamaName}". Please follow up.${STOP_LINE}`,

  withdrawalApproved: (amount: number) =>
    `Your withdrawal of KES ${amount.toFixed(2)} has been approved and will be processed shortly.${STOP_LINE}`,

  chamaStarted: (chamaName: string, amount: number, frequency: string, memberNumber: number, payoutDate: string, startDate: string) =>
    `"${chamaName}" has officially started on ${startDate}. Contribute KES ${amount.toLocaleString()} ${frequency}. You are member #${memberNumber}. Your payout date: ${payoutDate}.${STOP_LINE}`,

  paymentReminder: (chamaName: string, amount: number, dueDate: string, memberCode: string) =>
    `Reminder — Your contribution of KES ${amount.toLocaleString()} for "${chamaName}" is due on ${dueDate}. Member ID: ${memberCode}. Pay on time to secure your payout.${STOP_LINE}`,

  cycleComplete: (chamaName: string, managerName: string, managerPhone: string, memberCode: string) =>
    `"${chamaName}" has completed its full cycle. All payouts have been made. To rejoin, contact ${managerName} at ${managerPhone} or log in to the app. Member ID: ${memberCode}.${STOP_LINE}`,

  rejoinRequestSubmitted: (memberName: string, chamaName: string) =>
    `${memberName} has submitted a rejoin request for "${chamaName}". Log in to approve or reject it.${STOP_LINE}`,

  rejoinApproved: (chamaName: string) =>
    `Your rejoin request for "${chamaName}" has been approved. You will be notified when the new cycle begins.${STOP_LINE}`,

  rejoinRejected: (chamaName: string, reason?: string) =>
    `Your rejoin request for "${chamaName}" was not approved. ${reason || 'Contact the manager for details.'}${STOP_LINE}`,
