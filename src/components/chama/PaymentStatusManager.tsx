...
export const PaymentStatusManager = ({
  ...
  const getDateRange = (p: PeriodType) => {
    ...
    switch (p) {
      case "today": {
        ...
        return { start: todayCycleStart, end: todayCycleEnd };
      }
      case "week":
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      // Track document for serial & QR
      const { serialNumber, documentId } = await trackDocumentWithId({
        documentType: "payment_status_report",
        documentTitle: `${chamaName} - Payment Status ${getPeriodLabel(pdfPeriod)}`,
        metadata: { period: pdfPeriod, paidCount: paidMembersForPdf.length, unpaidCount: unpaidMembersForPdf.length },
      });
      ...
      // Branded footer with QR code and serial
      ...
      <Card className="border-primary/30 bg-primary/5">
        ...
        <CardContent>
          <p className="text-sm text-muted-foreground">
            ...
            Once started, members will have until 9:00 PM the next day
